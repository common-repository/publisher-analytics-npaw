...
function publisher_analytics_admin_init(){
    ...
    add_settings_field('publisher_analytics_flicker_control', 'Flicker Control', 'publisher_analytics_setting_flicker_control', 'publisher_analytics', 'publisher_analytics_main');
    add_settings_field('publisher_analytics_custom_selectors', '', 'publisher_analytics_setting_custom_selectors', 'publisher_analytics', 'publisher_analytics_main');
}

function publisher_analytics_section_text() {
    echo '<p>If your template is not yet on the list of integrated templates or you do not yet have an account code, please contact our support team at rasmus.sorensen@nicepeopleatwork.com</p><p>For more information, go to https://publisheranalytics.ai/docs</p>';
}
...
function publisher_analytics_setting_template() {
    ...
    <script>
        function showCustomFields(value) {
            ...
            document.getElementById('customSelectorsTitle').style.display = display;
        }
...
function publisher_analytics_setting_flicker_control() {
    $options = get_option('publisher_analytics_options');
    $flicker_control = isset($options['flicker_control']) ? $options['flicker_control'] : 'false';
    ?>
    <select id='publisher_analytics_flicker_control' name='publisher_analytics_options[flicker_control]'>
        <option value='false' <?php selected($flicker_control, 'false'); ?>>False</option>
        <option value='true' <?php selected($flicker_control, 'true'); ?>>True</option>
    </select>
    <?php
}
...
function publisher_analytics_setting_custom_selectors() {
    ...
    echo "<div id='customSelectorsTitle' style='display:none;'><h3>Custom Selectors</h3></div>";
...
function publisher_analytics_options_validate($input) {
    ...
    $newinput['flicker_control'] = sanitize_text_field($input['flicker_control']);
...
function publisher_analytics_get_inline_script() {
    ...
    ob_start();
    ?>
    <!-- Publisher Analytics WordPress-Plugin Version 1.2.1 -->

    const accountCode = '<?php echo esc_js($options['account_code']); ?>';
    const flickerControl = <?php echo esc_js($options['flicker_control']); ?>;
    const sdk = new PublisherAnalyticsSDK(accountCode, flickerControl);
